...
namespace {
  ...
  void append_html( const string & src, string & target ) {
    ...
    // On some platforms (e.g. tru64cxx) the following line is a real
    // performance boost
  ...
  string test_path_to_library_name( string const& path ) {
    ...
    if ( start_pos != string::npos ) {
      ...
      // So, the part between "libs" and "test/something.test" can
      // be considered as library name. But, for some libraries
      // tests are located too deep, say numeric/ublas/test/test1
      // directory, and some libraries have tests in several
      // subdirectories (regex/example and regex/test). So, nested
  ...
  string::size_type parse_skipped_msg_aux(const string& msg,
    ...
    if ( boost_build_v2 ) {
      ...
      if (dir.find("../") == 0) {
        ...
        // dir is always relative to the boost directory tree
  ...
  public:
    ...
    const string & target_directory() const {
      return m_target_directory;
    }
