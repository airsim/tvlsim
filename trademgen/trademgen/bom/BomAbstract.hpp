#ifndef __TRADEMGEN_BOM_BOMABSTRACT_HPP
#define __TRADEMGEN_BOM_BOMABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRADEMGEN::BomAbstract& iBom) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRADEMGEN::BomAbstract& ioBom) {
  // Fill Bom object with input stream
  ioBom.fromStream (ioIn);
  return ioIn;
}

#endif // __TRADEMGEN_BOM_BOMABSTRACT_HPP
