#ifndef __TRADEMGEN_DBA_DBAABSTRACT_HPP
#define __TRADEMGEN_DBA_DBAABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRADEMGEN::DbaAbstract& iDba) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRADEMGEN::DbaAbstract& ioDba) {
  // Fill Dba object with input stream
  ioDba.fromStream (ioIn);
  return ioIn;
}

#endif // __TRADEMGEN_DBA_DBAABSTRACT_HPP
