...
#
#   DSim client in Python
#   Connects REQ socket to tcp://localhost:5555
#   Sends JSON-ified parameters of a flight-date to display to the DSim 
...
def main():
    ...
    print "Connecting to DSim server..."
    context = zmq.Context()
    socket = context.socket (zmq.REQ)
    socket.connect ("tcp://localhost:5555")
    print "... done"

    #  Send the flight-date parameters to the DSim server
    print "Requesting flight-date ", jsonActionString, " from the DSim server..."
    socket.send (jsonActionString)
    
    #  Get the flight-date details from the DSim server
