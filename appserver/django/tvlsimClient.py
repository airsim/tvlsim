...
#
#   TvlSim client in Python
#   Connects REQ socket to tcp://localhost:5555
#   Sends JSON-ified parameters of a flight-date to display to the TvlSim 
...
def main():
    ...
    print "Connecting to TvlSim server..."
    context = zmq.Context()
    socket = context.socket (zmq.REQ)
    socket.connect ("tcp://localhost:5555")
    print "... done"

    #  Send the flight-date parameters to the TvlSim server
    print "Requesting flight-date ", jsonActionString, " from the TvlSim server..."
    socket.send (jsonActionString)
    
    #  Get the flight-date details from the TvlSim server
