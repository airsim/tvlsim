  ...
  priceQuote (const stdair::SegmentPath_T& iSegmentPath,
	...
	if (hasParsingBeenSuccesful) {
	  ...
	  if (lAirportPair_ptr == NULL) {
          ...
          // solution
          const FarePosChannel* lFarePosChannel_ptr = stdair::BomManager::
          ...
          if (lFarePosChannel_ptr == NULL) {
            STDAIR_LOG_ERROR ("No available fare rule corresponding to the position "
                              << lPosition << ", to the channel "
                              << lChannel << " and to the couple Origin-Destination "
                              << lAirportPairString);
            throw PositionNotFoundException ("No available fare rule for the position "
                                             + lPosition + " and the channel " + lChannel);
          }
          ...
          const stdair::Duration_T lSPTime(boost::posix_time::duration_from_string(lResultParsing.at(3)));
