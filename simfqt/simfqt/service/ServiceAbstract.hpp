#ifndef __SIMFQT_SVC_SERVICEABSTRACT_HPP
#define __SIMFQT_SVC_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const SIMFQT::ServiceAbstract& iService) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            SIMFQT::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __SIMFQT_SVC_SERVICEABSTRACT_HPP
