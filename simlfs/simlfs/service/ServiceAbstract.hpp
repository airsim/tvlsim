#ifndef __SIMLFS_SVC_SERVICEABSTRACT_HPP
#define __SIMLFS_SVC_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const SIMLFS::ServiceAbstract& iService) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            SIMLFS::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __SIMLFS_SVC_SERVICEABSTRACT_HPP
