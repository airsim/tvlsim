 ...
 *
 * events are allocated and deallocated, while periods are not (to allow multiple usages of the same generator)
 */
...
int get_next_event(struct world *wd, struct event **event)
{
...
int init_world (struct world *wd, char * arrivalPatterns)
{
  struct generator **generators;
  int ngenerators; 
