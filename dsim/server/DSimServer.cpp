/*!
 * \page DSimServer.cpp Server to Demonstrate Typical DSim Usage
 * \code
 */
// //////////////////////////////////////////////////////////////////////
// Import section
 ...
 */
const std::string K_DSIM_DEFAULT_LOG_FILENAME ("dsimServer.log");

/**
 * Default protocol to use for the server.
 */
const std::string K_DSIM_DEFAULT_SERVER_PROTOCOL ("tcp://");

/**
 * Default address to bind to.
 */
const std::string K_DSIM_DEFAULT_SERVER_ADDRESS ("*");

/**
 * Default port to bind to.
 */
const ServerPort_T K_DSIM_DEFAULT_SERVER_PORT (5555);
 ...
 */
const std::string K_DSIM_DEFAULT_FRAT5_INPUT_FILENAME (STDAIR_SAMPLE_DIR
                                                       "/frat5.csv");
/**
 * Fare family disutility curve input file name
 */
const std::string K_DSIM_DEFAULT_FF_DISUTILITY_INPUT_FILENAME (STDAIR_SAMPLE_DIR
                                                               "/ffDisutility.csv");
...
int readConfiguration (int argc, char* argv[], std::string& ioServerProtocol,
  ...
  config.add_options()
    ...
    ("protocol,t",
     boost::program_options::value< std::string >(&ioServerProtocol)->default_value(K_DSIM_DEFAULT_SERVER_PROTOCOL),
     "Server protocol")
    ("address,a",
     boost::program_options::value< std::string >(&ioServerAddress)->default_value(K_DSIM_DEFAULT_SERVER_ADDRESS),
     "Server address")
    ("port,p",
     boost::program_options::value< ServerPort_T >(&ioServerPort)->default_value(K_DSIM_DEFAULT_SERVER_PORT),
     "Server port")
    ("log,l",
...
int main (int argc, char* argv[]) {
  ...
  // DEBUG
  STDAIR_LOG_DEBUG ("The DSim server is ready to receive requests...");
