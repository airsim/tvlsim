...
namespace services {
  ...
  /// Debugging stream socket service that wraps the normal stream
  /// socket service.
  ...
  public:
    ...
    public:
      connect_handler(Handler h, logger& l) : handler_(h), logger_(l) {
      }
    ...
    class send_handler {
    public:
      send_handler(Handler h, logger& l) : handler_(h), logger_(l) {
    }
