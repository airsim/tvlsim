#ifndef __DSIM_SVC_SERVICEABSTRACT_HPP
#define __DSIM_SVC_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const DSIM::ServiceAbstract& iService) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            DSIM::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __DSIM_SVC_SERVICEABSTRACT_HPP
