#ifndef __AVLCAL_SVC_SERVICEABSTRACT_HPP
#define __AVLCAL_SVC_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const AVLCAL::ServiceAbstract& iService) {
  /**
     string stream:
     - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            AVLCAL::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __AVLCAL_SVC_SERVICEABSTRACT_HPP
