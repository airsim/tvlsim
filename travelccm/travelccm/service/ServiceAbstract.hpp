#ifndef __TRAVELCCM_SERVICEABSTRACT_HPP
#define __TRAVELCCM_SERVICEABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRAVELCCM::ServiceAbstract& iService) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRAVELCCM::ServiceAbstract& ioService) {
  // Fill Service object with input stream
  ioService.fromStream (ioIn);
  return ioIn;
}

#endif // __TRAVELCCM_SERVICEABSTRACT_HPP
