#ifndef __TRAVELCCM_BOM_BOMABSTRACT_HPP
#define __TRAVELCCM_BOM_BOMABSTRACT_HPP
...
operator<< (std::basic_ostream<charT, traits>& ioOut,
            const TRAVELCCM::BomAbstract& iBom) {
  /**
     string stream:
      - with same format
...
operator>> (std::basic_istream<charT, traits>& ioIn,
            TRAVELCCM::BomAbstract& ioBom) {
  // Fill Bom object with input stream
  ioBom.fromStream (ioIn);
  return ioIn;
}

#endif // __TRAVELCCM_BOM_BOMABSTRACT_HPP
