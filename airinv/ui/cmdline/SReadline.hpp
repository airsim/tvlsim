...
/**
 * @brief Used to avoid linking errors in case of including into many
 * compilation units
 */
namespace {
  /**
  ...
  /**
   * @brief The function selects the set of bindings and makes the
   * corresponding call.
   *
  ...
  /**
   * @brief The readline startup hook. It is required to setup the
   * proper keymap.
   */
  ...
  bool AreTokensEqual (const Container& Pattern, const Container& Input) {
    ...
    for ( ; j != Input.end(); ++k, ++j) {
      const std::string lPattern = *k;
      if (lPattern == "%file") {
        continue;
      }

      const std::string lInput = *j;
      if (lPattern != lInput) {
        return false;
      }
    }
  ...
  void SplitTokens (const std::string& Source, ContainerType& Container) {
    ...
    // Tokens provider
    TokenizerType Tokenizer (Source, Separators);
        
    Container.clear();
    for (TokenizerType::const_iterator k (Tokenizer.begin());
         k != Tokenizer.end(); ++k) {
      // Temporary storage for the token, in order to trim that latter
      std::string SingleToken (*k);
  ...
  char** UserCompletion (const char* text, int start, int end) {
    ...
    if (Completions.empty() == true) {
      return NULL;
    }

    // Memorise all the previous tokens
    std::string PreInput (rl_line_buffer, start);
    SplitTokens (PreInput, Tokens);

    // Detect whether we should call the standard file name completer
    // or a custom one
    bool FoundPretender (false);

    for (CompletionsStorage::const_iterator k (Completions.begin());
         k != Completions.end(); ++k) {
      const TokensStorage& lTokenStorage = *k;
      if (AreTokensEqual (lTokenStorage, Tokens) == false) {
        continue;
      }

      if (lTokenStorage.size() > Tokens.size()) {
        FoundPretender = true;
        if (lTokenStorage [Tokens.size()] == "%file") {
  ...
  char* Generator (const char* text, int State) {
    ...
    for ( ; Iterator != Completions.end(); ++Iterator) {
      const TokensStorage& lCompletion = *Iterator;
      if (AreTokensEqual (lCompletion, Tokens) == false) {
        continue;
      }

      if (lCompletion.size() > Tokens.size()) {
        if (lCompletion [Tokens.size()] == "%file") {
          continue;
        }
            
        const char* lCompletionCharStr (lCompletion [Tokens.size()].c_str());
        if (strncmp (text, lCompletionCharStr, Length) == 0) {
          // Readline will free the allocated memory
          const size_t lCompletionSize = strlen (lCompletionCharStr) + 1;
          char* NewString (static_cast<char*> (malloc (lCompletionSize)));
          strcpy (NewString, lCompletionCharStr);
...
namespace swift {
  ...
  */
  ...
  public:
     ...
     *
     * @param PrintableBound if true - the printable characters are
     * bound if false - the keymap is empty
     */
    explicit SKeymap (bool PrintableBound = false) : keymap (NULL) {
      if (PrintableBound == true) {
        // Printable characters are bound
     ...
     */
    explicit SKeymap (Keymap Pattern) : keymap (rl_copy_keymap (Pattern)) {
      if ( keymap == NULL ) {
        throw std::runtime_error( "Cannot allocate keymap." );
      }
  ...
  public:
    ...
    /**
     * @brief Saves the session history (if the file name was
     * provided) and destroys the object
     */
    ~SReadline() {
    ...
    /**
     * @brief Clears the history. Does not affect the file where the
     * previous session history is saved.
    */
    void ClearHistory() {
      clear_history();
    }
     ...
     *
     * Supports a special keyword: %file. It means to use the standard
     * file name completer.
     *
     * For example the given container elements could be as follows:
     * <ul>
     * <li>command1 opt1</li>
     * <li>command1 opt2 %file</li>
     * <li>command2</li>
     * <li>command2 opt1</li>
     * </ul>
     *
    ...
    void RegisterCompletions (const ContainerType& Container) {
      ...
      for (typename ContainerType::const_iterator k (Container.begin());
        ...
        const std::string& kStr = static_cast<std::string> (*k);

        SplitTokens (kStr, OneLine);
  ...
  private:
    // /////////////////////////// Attributes /////////////////////////
    /**
     * Constructor accepted or default.
     */
    const size_t HistoryLimit;

    /**
     * Original constructor accepted.
     */
